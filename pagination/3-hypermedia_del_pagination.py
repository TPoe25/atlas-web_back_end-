...
class Server:
    ...
    def get_hyper_index(self, page: int = 1, page_size: int = 10) -> Dict:
        ...
        - index: a dictionary containing the following key-value pairs:
            - page_size: the length of the returned dataset page
            - page: the current page number
            - data: the dataset page (equivalent to return from previous task)
        - next_index: number of the next page, None if no next page
        - prev_index: number of the previous page, None if no previous page
        - total_pages: the total number of pages in the dataset as an integer
